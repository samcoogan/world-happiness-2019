...
app.layout = dbc.Container([
    html.Div(className='app-header', children=[
        html.H1("World Happiness Index - 2019", className='display-3')
        ]),
    dbc.Row([
        dbc.Col([
            dcc.Dropdown(
    ...
    dbc.Row([dbc.Col(dcc.Graph(id='world-map'), width=12)]),
    dbc.Row([dbc.Col([
            html.Div(id='data-insights', className='data-insights'),
            html.Div(id='top-bottom-countries', className='top-bottom-countries')], width=8),
...
def update_map(selected_metric):
    fig = px.choropleth(df,locations="Country or region",locationmode="country names", color=selected_metric, hover_name="Country or region", color_continuous_scale=px.colors.sequential.Aggrnyl_r,title=f'World Happiness Index:{selected_metric}')

    fig.update_layout(margin={"r":0,"t":40, "l":0, "b":40})
    return fig

# Top and bottom country for each input
@app.callback(
...
def update_insights(selected_metric):
    ...
    insights = [html.H3(selected_metric),
                html.H3(f"Highest: {highest['Country or region']} ({highest[selected_metric]})"),
                html.H3(f"Lowest: {lowest['Country or region']} ({lowest[selected_metric]})")
    ]
...
def update_top_bottom(selected_metric):
    ...
    return html.Div([
        ...
        html.Div([html.H3("Bottom 5 Countries"), bottom_countries_list], className="top-bottom-section")
        ])


# Click country to display info
@app.callback(
    ...
    if clickData:
        ...
        if not country_data.empty:
            ...
            details = [
                ...
                html.P(f"Free to make life choices: {country['Freedom to make life choices']}"),
